...
export default class Run extends Component {
  ...
  onSetPreScript = d => {
    this.setState({
      pre_script_ajax: d.text
    });
  };

  onSetAfterScript = d => {
    this.setState({
      after_script_ajax: d.text
    });
  };
  ...
  reqRealInterface = async () => {
    ...
    options.pre_script_ajax = this.state.pre_script_ajax;
    options.after_script_ajax = this.state.after_script_ajax;
        ...
        <Tabs size="large" defaultActiveKey="res" className="response-tab">
          ...
          {this.props.type === 'case' ? (
            <Tabs.TabPane
              ...
              tab={<Tooltip title="请求前拦截">pre_script</Tooltip>}
              key="pre_script"
            >
              <p style={{ margin: '10px' }}>注：脚本只有做自动化测试才执行</p>
              <p style={{ margin: '10px' }}>注：先执行全局的pre_script，再执行本脚本，共享context</p>
              <Row>
                <Col span="24">
                  <AceEditor
                    onChange={this.onSetPreScript}
                    className="case-script"
                    data={this.state.pre_script_ajax}
                  />
                </Col>
              </Row>
            </Tabs.TabPane>
          ...
          {this.props.type === 'case' ? (
            <Tabs.TabPane
              className="response-test"
              tab={<Tooltip title="请求后拦截">after_script</Tooltip>}
              key="after_script"
            >
              <p style={{ margin: '10px' }}>注：脚本只有做自动化测试才执行</p>
              <p style={{ margin: '10px' }}>注：先执行全局的after_script，再执行本脚本，共享context</p>
              <Row>
                <Col span="24">
                  <AceEditor
                    onChange={this.onSetAfterScript}
                    className="case-script"
                    data={this.state.after_script_ajax}
                  />
                </Col>
              </Row>
            </Tabs.TabPane>
