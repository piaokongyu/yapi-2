...
class InterfaceColContent extends Component {
  ...
  executeTests = async () => {
    for (let i = 0, l = this.state.rows.length, newRows, curitem; i < l; i++) {
      ...
      curitem = Object.assign(
        ...
        {
          env: envItem.env,
          pre_script: `${this.props.currProject.pre_script};${rows[i].pre_script_ajax || ''};`,
          after_script: `${this.props.currProject.after_script};${rows[i].after_script_ajax || ''};`
        },
        { test_status: 'loading' },
  ...
  handleTest = async interfaceData => {
    ...
    options.records = interfaceData.records || {};
    let result = {
      code: 400,
      msg: '数据异常',
      validRes: []
