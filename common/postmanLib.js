...
function sandboxByBrowser(context = {}, script) {
  ...
  // (function() {})()
  try {
    eval(`(function(window, document) {${beginScript}${script}})(window, document)`);
  } catch (err) {
    let message = `Script:
                   ----CodeBegin----:
                   ${beginScript}
...
async function crossRequest(defaultOptions, preScript, afterScript, commonContext = {}) {
  ...
  if (options.pre_script_ajax) preScript += `;${options.pre_script_ajax};`;
  if (options.after_script_ajax) afterScript += `;${options.after_script_ajax};`;
